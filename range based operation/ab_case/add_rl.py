...
super_ = [[5, 15, 31], [-44, 36, 51], [-68, 28, 38], [71, 45, 52], [-171, 3, 24], [-30, 26, 37], [81, 16, 24], [82, 47, 54], [81, 3, 6]]
train_choice += test_choice
print train_choice
data_choice = test_choice
cur_x = np.array(dataset[data_choice[9]][3])
dst_x = np.array(dataset[data_choice[9]][4])
print dataset[data_choice[9]][5]
print dst_x
ori_dis = np.linalg.norm(cur_x - dst_x)
ori_op = dataset[data_choice[9]][0] * 2 - 1
min_op = dataset[data_choice[9]][0] * 2 - 1
def greedy_search(x):
    global dst_x
...
def cal_state(s_op, depth, filter_):
    ...
    for i,(pk_op_c, pk_op_i, start_id, end_id) in enumerate(s_op):
        s_[start_id:end_id] = s_[start_id:end_id]*pk_op_c + pk_op_i
	#print s_op[i]
#	s_op[i][1] *= -1
    ...
    choice = np.array([(len(candidate_op[i][4]), i) for i in range( len(candidate_op) )])
    ...
    for i in range( min(20, len(candidate_op)) ):
        ...
        true_op.append( candidate_op[i][:-1] )
    ...
    def train_super(self):
        ...
        for i, j in zip(self.replay_x, self.states):
            for jj in j:
                i.append(list(jj))
        self.replay_y += list(Y)
        self.states, self.probs, self.gradients, self.rewards, self.labels = [], [], [], [], []
        self.model.train_on_batch([np.array(i) for i in self.replay_x], np.array(self.replay_y))
        for i in range(state_shape[0]):
            self.states += [[]]
...
class Maze(object):
    ...
    def trace(self, supervise=False):
        global trie, stateT, cnt, curve, best, min_op
        mx_op, mx_para = [], []
        mx_score = -1000 
        batch = 0
        para_s, score = [[], [], [], [], [], []], 0
