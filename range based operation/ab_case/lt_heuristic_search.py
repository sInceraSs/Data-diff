...
def weigh(cur_x, depth):
	...
	diff_op, diff_blocks = len(tmp_op) + 1, len(cur_op)
	if diff_blocks == 0:
		return depth, diff_blocks, assign
	# weight = dist * depth * 10
	#weight =  np.log2(dist*dist_1) * (100)
	weight = np.log2(diff_op*depth) * (100)
	return (weight, diff_blocks, assign)
...
def heuristic_search(pre_op, depth, cur_weight, test_idx):
	...
	while True:
		...
		if np.allclose(cur_x, ori_x):
			...
			if len(pre_op) == test_idx:
				ans.sort()
				ans_v[test_idx].append(clock() - start_t)
...
def render_state_tree(stateT):
	...
	fn = g1.render(filename='search\wc_md\\try_herustic_v5')
...
"""
# open("ans_lt", "w").write(json.dumps( ans_v ))